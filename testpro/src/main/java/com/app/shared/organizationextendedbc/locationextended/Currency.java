...
@SourceCodeAuthorClass(createdBy = "deepali.arvind@algorhythm.co.in", updatedBy = "deepali.arvind@algorhythm.co.in", versionNumber = "3", comments = "Currency", complexity = Complexity.LOW)
@NamedQueries({ @javax.persistence.NamedQuery(name = "Currency.findAll", query = " select u from Currency u where u.systemInfo.activeStatus=1"), @javax.persistence.NamedQuery(name = "Currency.DefaultFinders", query = "select e from Currency e where e.systemInfo.activeStatus=1 and e.currencyId LIKE :currencyId and e.countryId LIKE :countryId"), @javax.persistence.NamedQuery(name = "Currency.findByCountryId", query = "select e from Currency e where e.systemInfo.activeStatus=1 and e.countryId=:countryId"), @javax.persistence.NamedQuery(name = "Currency.findById", query = "select e from Currency e where e.systemInfo.activeStatus=1 and e.currencyId =:currencyId") })
public class Currency implements Serializable, CommonEntityInterface, Comparator<Currency> {

    private static final long serialVersionUID = 1474436566219L;
