Ext.define('Testpro.testpro.web.com.controller.appinsight.health.BEntityMainController', {
     extend: 'Testpro.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.BEntityMainController',
     init: function() {
          var form = this.view.down('#BEntityForm');
          form.reset();
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#BEntityGrid');
          var tree = this.view.down('#BEntityTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#BEntity');
          var grid = this.view.down('#BEntityGrid');
          var tree = this.view.down('#BEntityTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#BEntityForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#BEntityTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var BEntity = this.view.down('#BEntityForm');
               BEntity.reset();
               var gridPanel = this.view.down('#BEntityGrid');
          ...
          var formPanel = this.getView().up().down('#BEntityForm');
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Testpro.testpro.shared.com.model.appinsight.health.BEntityModel',
               autoLoad: true,
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#BEntityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#BEntity').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#BEntityGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
