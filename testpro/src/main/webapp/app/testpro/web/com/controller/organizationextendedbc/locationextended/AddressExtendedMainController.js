Ext.define('Testpro.testpro.web.com.controller.organizationextendedbc.locationextended.AddressExtendedMainController', {
     extend: 'Testpro.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.AddressExtendedMainController',
     init: function() {
          var form = this.view.down('#AddressExtendedForm');
          form.reset();
     ...
     onDistrictIdChange: function(me, newValue, oldValue, eOpts) {
          this.onDistrictIdChangeVillageId(me, newValue, oldValue, eOpts);
          this.onDistrictIdChangeTalukaId(me, newValue, oldValue, eOpts);
     },
     onRegionIdChange: function(me, newValue, oldValue, eOpts) {
          this.onRegionIdChangeVillageId(me, newValue, oldValue, eOpts);
          this.onRegionIdChangeTalukaId(me, newValue, oldValue, eOpts);
          this.onRegionIdChangeDistrictId(me, newValue, oldValue, eOpts);
     },
     onDistrictIdChangeVillageId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Village/findByDistrictId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               success: function(response, scope) {
                    var villageIdCombo = scope.element.up('form').down('#villageId');
                    villageIdCombo.setValue();
                    var responseText = Ext.JSON.decode(response.responseText);
                    villageIdCombo.setComboData(responseText.response.data);
                    scope.sender.controller.addCommunicationLog(response, true, villageIdCombo);
               },
     ...
     onDistrictIdChangeTalukaId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               ...
               element: me,
               success: function(response, scope) {
                    var talukaIdCombo = scope.element.up('form').down('#talukaId');
                    talukaIdCombo.setValue();
                    var responseText = Ext.JSON.decode(response.responseText);
                    talukaIdCombo.setComboData(responseText.response.data);
                    scope.sender.controller.addCommunicationLog(response, true, talukaIdCombo);
               },
               failure: function(response, scope) {
                    scope.sender.controller.addCommunicationLog(response, false);
               }
     ...
     onRegionIdChangeVillageId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/Village/findByRegionId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
     ...
     onRegionIdChangeTalukaId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
     ...
     onRegionIdChangeDistrictId: function(me, newValue, oldValue, eOpts) {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    ...
                    districtIdCombo.setValue();
                    var responseText = Ext.JSON.decode(response.responseText);
                    districtIdCombo.setComboData(responseText.response.data);
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#AddressExtendedGrid');
          var tree = this.view.down('#AddressExtendedTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#AddressExtended');
          var grid = this.view.down('#AddressExtendedGrid');
          var tree = this.view.down('#AddressExtendedTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#AddressExtendedForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, opts) {
                    ...
                    var compRef = currentView.down('#AddressExtendedTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var AddressExtended = this.view.down('#AddressExtendedForm');
               AddressExtended.reset();
               var gridPanel = this.view.down('#AddressExtendedGrid');
          ...
          var formPanel = this.getView().up().down('#AddressExtendedForm');
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Testpro.testpro.shared.com.model.organizationextendedbc.locationextended.AddressExtendedModel',
               autoLoad: true,
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#AddressExtendedTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AddressExtended').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AddressExtendedGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
