Ext.define('Testpro.testpro.web.com.controller.organizationextendedbc.locationextended.ZonesMainController', {
     extend: 'Testpro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ZonesMainController',
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ZonesForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ZonesGrid');
               var tree = this.view.down('#ZonesTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ZonesForm');
          var grid = this.view.down('#ZonesGrid');
          var tree = this.view.down('#ZonesTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ZonesGrid');
               ...
               var formPanel = this.getView().up().down('#ZonesForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#ZonesTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ZonesForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Testpro.testpro.shared.com.model.organizationextendedbc.locationextended.ZonesModel',
               autoLoad: true,
