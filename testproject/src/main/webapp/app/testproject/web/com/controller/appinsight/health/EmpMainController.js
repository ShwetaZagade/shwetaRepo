Ext.define('Testproject.testproject.web.com.controller.appinsight.health.EmpMainController', {
     extend: 'Testproject.view.fw.frameworkController.AggregateViewController',
     ...
     resetForm: function(but) {
          ...
          tree.setSelection();
          grid.setSelection();
          var gridEmpDEpt = but.up('form').down('#EmpDEptGrid');
          gridEmpDEpt.getStore().removeAll();
          gridEmpDEpt.reconfigure();
     },
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          var EmpDEptGrid = formPanel.down('#EmpDEptGrid');
          EmpDEptGrid.store.loadData(data.empDEpt);
          EmpDEptGrid.setSelection(0);
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Testproject.testproject.shared.com.model.appinsight.health.EmpModel',
               autoLoad: true,
     ...
     addEmpDEpt: function(but) {
          var formEmpDEpt = but.up().down('form');
          var gridEmpDEpt = but.up().up().down('#EmpDEptGrid');
          if (formEmpDEpt.isValid()) {
               var values = this.getData(formEmpDEpt);
               values = values[gridEmpDEpt.bindable];
               if (gridEmpDEpt.selection) {
                    gridEmpDEpt.selection.data = values;
               } else {
                    gridEmpDEpt.getStore().add(values);
               }
               gridEmpDEpt.reconfigure();
               gridEmpDEpt.setSelection();
               formEmpDEpt.reset();
          }
     },
     onEmpDEptGridItemClick: function(cellModel, record, rowIndex, columnIndex, eOpts, comp) {
          var form;
          if (comp != null) {
               form = comp.up();
          } else {
               form = this.view.down('#EmpDEptForm');
          }
     ...
     saveForm: function(but, evt) {
          ...
          var EmpDEptForm = form.down('#EmpDEptForm');
          var EmpDEpt = form.down('#EmpDEptGrid');
          if (!(EmpDEptForm.isValid() || EmpDEpt.store.getCount() != 0)) {
               this.showCard(form, EmpDEptForm);
               return;
          }
          this.addEmpDEpt(EmpDEptForm.down('button'));
